...
import type { ValidationIssue, ValidationSeverity } from "@core/domain/types/validation.js";
...
const validateGameStructure = (game: Game): ValidationIssue[] => {
  ...
  if (!isNonEmptyString(game.id)) {
    issues.push(issue("id", "INVALID_ID", "Game id must be a non-empty string"));
  }
  if (!isNonNegativeInteger(game.turn)) {
    issues.push(issue("turn", "INVALID_TURN", "Turn must be a non-negative integer"));
  }
  if (!Array.isArray(game.dominoes)) {
    issues.push(issue("dominoes", "INVALID_DOMINOES", "Dominoes must be an array"));
  }
  if (!Array.isArray(game.currentDominoes)) {
    issues.push(issue("currentDominoes", "INVALID_CURRENT_DOMINOES", "CurrentDominoes must be an array"));
  }
  if (!Array.isArray(game.players)) {
    issues.push(issue("players", "INVALID_PLAYERS", "Players must be an array"));
  }
  ...
  if (!game.rules || typeof game.rules !== "object") {
    ...
    if (!game.rules.basic || typeof game.rules.basic !== "object") {
      issues.push(issue("rules.basic", "INVALID_BASIC_RULES", "Rules.basic must be an object"));
    }
    if (!Array.isArray(game.rules.extra)) {
      issues.push(issue("rules.extra", "INVALID_EXTRA_RULES", "Rules.extra must be an array"));
    }
  }
  if (!game.mode || typeof game.mode !== "object") {
    ...
    if (!isNonEmptyString(game.mode.name)) {
      issues.push(issue("mode.name", "INVALID_MODE_NAME", "Mode name must be a non-empty string"));
    }
  }
  if (!game.nextAction || typeof game.nextAction !== "object") {
    issues.push(issue("nextAction", "INVALID_NEXT_ACTION", "NextAction must be an object"));
  } else if (game.nextAction.type !== "action" && game.nextAction.type !== "step") {
    issues.push(issue("nextAction.type", "INVALID_NEXT_ACTION_TYPE", "NextAction.type must be 'action' or 'step'"));
  }
...
const validateTile = (tile: unknown, path: string): ValidationIssue[] => {
  ...
  if (!tile || typeof tile !== "object") {
    issues.push(issue(path, "INVALID_TILE", `Tile at ${path} must be an object`));
    return issues;
  }
  const t = tile as Record<string, unknown>;
  if (typeof t.type !== "string" || !validGrounds.has(t.type)) {
    issues.push(issue(`${path}.type`, "INVALID_TILE_TYPE", `Invalid terrain type at ${path}`));
  }
  if (typeof t.crowns !== "number" || t.crowns < 0) {
    issues.push(issue(`${path}.crowns`, "INVALID_TILE_CROWNS", `Invalid crowns at ${path}`));
  }
  return issues;
};
...
const validatePlayers = (game: Game): ValidationIssue[] => {
  ...
  for (let i = 0; i < game.players.length; i++) {
    ...
    if (!isNonEmptyString(player.id)) {
      issues.push(issue(`${path}.id`, "INVALID_PLAYER_ID", `Player id must be a non-empty string`));
    } else if (playerIds.has(player.id)) {
      issues.push(issue(`${path}.id`, "DUPLICATE_PLAYER_ID", `Duplicate player id: ${player.id}`));
    } else {
      playerIds.add(player.id);
    }

    if (typeof player.name !== "string" || player.name.length < 3) {
      issues.push(issue(`${path}.name`, "INVALID_PLAYER_NAME", `Player name must be at least 3 characters`));
    }

    // Kingdom validation
    if (!Array.isArray(player.kingdom)) {
      issues.push(issue(`${path}.kingdom`, "INVALID_KINGDOM", `Kingdom must be a 2D array`));
      continue;
    }
    if (player.kingdom.length !== GRIDSIZE) {
      issues.push(issue(`${path}.kingdom`, "INVALID_KINGDOM_GRID", `Kingdom must have ${GRIDSIZE} rows, got ${player.kingdom.length}`));
      continue;
    }
    ...
    for (let y = 0; y < player.kingdom.length; y++) {
      ...
      if (!Array.isArray(row) || row.length !== GRIDSIZE) {
        issues.push(issue(`${path}.kingdom[${y}]`, "INVALID_KINGDOM_ROW", `Kingdom row ${y} must have ${GRIDSIZE} columns`));
        continue;
      }
      for (let x = 0; x < row.length; x++) {
        const cell = row[x]!;
        issues.push(...validateTile(cell, `${path}.kingdom[${y}][${x}]`));
        if (cell && typeof cell === "object" && (cell as Record<string, unknown>).type === "castle") {
          castleCount++;
        }
      }
    }
    if (castleCount === 0) {
      issues.push(issue(`${path}.kingdom`, "MISSING_CASTLE", `Kingdom must have exactly one castle tile`, "warning"));
    } else if (castleCount > 1) {
      issues.push(issue(`${path}.kingdom`, "MULTIPLE_CASTLES", `Kingdom has ${castleCount} castle tiles, expected 1`));
    }
  }
  ...
  if (Array.isArray(game.dominoes)) {
    for (let i = 0; i < game.dominoes.length; i++) {
      ...
      if (!isPositiveInteger(domino.number)) {
        issues.push(issue(`${path}.number`, "INVALID_DOMINO_NUMBER", `Domino number must be a positive integer`));
      } else if (allNumbers.has(domino.number)) {
        issues.push(issue(`${path}.number`, "DUPLICATE_DOMINO_NUMBER", `Duplicate domino number: ${domino.number}`));
      } else {
  ...
  if (Array.isArray(game.currentDominoes)) {
    for (let i = 0; i < game.currentDominoes.length; i++) {
      ...
      if (!revealed.domino || typeof revealed.domino !== "object") {
        issues.push(issue(`${path}.domino`, "INVALID_REVEALED_DOMINO", `Revealed domino must be an object`));
        continue;
      }

      const domino = revealed.domino;
      if (!isPositiveInteger(domino.number)) {
        issues.push(issue(`${path}.domino.number`, "INVALID_DOMINO_NUMBER", `Domino number must be a positive integer`));
      } else if (allNumbers.has(domino.number)) {
        issues.push(issue(`${path}.domino.number`, "DUPLICATE_DOMINO_NUMBER", `Duplicate domino number: ${domino.number}`));
      } else {
        allNumbers.add(domino.number);
      }
      ...
      if (revealed.picked && revealed.lordId === null) {
        issues.push(issue(`${path}`, "PICKED_WITHOUT_LORD", `Domino is marked as picked but has no lordId`));
      }
      if (!revealed.picked && revealed.lordId !== null) {
        issues.push(issue(`${path}`, "UNPICKED_WITH_LORD", `Domino is not picked but has a lordId`));
      }
    }
  }
    ...
    for (let i = 0; i < game.lords.length; i++) {
      ...
      if (lord.dominoPicked) {
        ...
        if (!isPositiveInteger(domino.number)) {
          ...
          issues.push(issue(`${path}.number`, "DUPLICATE_DOMINO_NUMBER", `Duplicate domino number among lords: ${domino.number}`));
        } else {
...
const validateLords = (game: Game): ValidationIssue[] => {
  ...
  for (let i = 0; i < game.lords.length; i++) {
    ...
    if (!isNonEmptyString(lord.id)) {
      issues.push(issue(`${path}.id`, "INVALID_LORD_ID", `Lord id must be a non-empty string`));
    } else if (lordIds.has(lord.id)) {
      issues.push(issue(`${path}.id`, "DUPLICATE_LORD_ID", `Duplicate lord id: ${lord.id}`));
    } else {
      lordIds.add(lord.id);
    }

    if (!isNonEmptyString(lord.playerId)) {
      issues.push(issue(`${path}.playerId`, "INVALID_LORD_PLAYER_ID", `Lord playerId must be a non-empty string`));
    } else if (playerIds.size > 0 && !playerIds.has(lord.playerId)) {
      issues.push(issue(`${path}.playerId`, "LORD_PLAYER_NOT_FOUND", `Lord references non-existent player: ${lord.playerId}`));
    }

    if (lord.turnEnded && (!lord.hasPick || !lord.hasPlace)) {
      issues.push(issue(path, "INCONSISTENT_LORD_FLAGS", `Lord has turnEnded=true but hasPick=${lord.hasPick}, hasPlace=${lord.hasPlace}`, "warning"));
    }
  }
...
const validateGameFlow = (game: Game): ValidationIssue[] => {
  ...
  if (nextAction.type === "action") {
    const action = nextAction as { type: "action"; nextLord?: unknown; nextAction?: unknown };

    if (!isNonEmptyString(action.nextLord)) {
      issues.push(issue("nextAction.nextLord", "INVALID_NEXT_LORD", "nextLord must be a non-empty string"));
    } else if (Array.isArray(game.lords) && !game.lords.some((l) => l.id === action.nextLord)) {
      issues.push(issue("nextAction.nextLord", "NEXT_LORD_NOT_FOUND", `nextLord references non-existent lord: ${action.nextLord as string}`));
    }

    if (typeof action.nextAction !== "string" || !validPlayerActions.has(action.nextAction as string)) {
      issues.push(issue("nextAction.nextAction", "INVALID_PLAYER_ACTION", `nextAction must be one of: ${[...validPlayerActions].join(", ")}`));
    }
  } else if (nextAction.type === "step") {
    const step = nextAction as { type: "step"; step?: unknown };

    if (typeof step.step !== "string" || !validGameSteps.has(step.step as string)) {
      issues.push(issue("nextAction.step", "INVALID_GAME_STEP", `step must be one of: ${[...validGameSteps].join(", ")}`));
    }
  }
...
const validateRules = (game: Game): ValidationIssue[] => {
  ...
  if (basic && typeof basic === "object") {
    const fields = ["lords", "maxDominoes", "dominoesPerTurn", "maxTurns", "maxKingdomSize"] as const;
    for (const field of fields) {
      const check = allowZero ? isNonNegativeInteger : isPositiveInteger;
      if (!check(basic[field])) {
        issues.push(issue(`rules.basic.${field}`, "INVALID_BASIC_RULE_VALUE", `rules.basic.${field} must be a positive integer`));
      }
    }
  }
  ...
  if (Array.isArray(game.rules.extra)) {
    for (let i = 0; i < game.rules.extra.length; i++) {
      ...
      if (!isNonEmptyString(rule.name)) {
        issues.push(issue(`rules.extra[${i}].name`, "INVALID_EXTRA_RULE_NAME", `Extra rule name must be a non-empty string`));
      }
    }
  }
