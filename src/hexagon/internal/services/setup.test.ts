...
describe("Game Setup", () => {
  ...
  test("should define rules for 2 players", () => {
    ...
    expect(newState.lords.length).toEqual(4);
  });
  ...
  test("should define rules for 3 players", () => {
    ...
    expect(newState.lords.length).toEqual(3);
  });
  ...
  test("should shuffle dominoes", () => {
    ...
    const deps = {
      ...dependencies,
      shuffleMethod: (array: any) => [...array].reverse(),
    };
  ...
  test("should setup players lords", () => {
    // Arrange
    const payload = {
      state,
      players: [{ name: "Player 1" }, { name: "Player 2" }],
    ...
    // Assert
    expect(newState.lords[0]?.playerId).toEqual(newState.players[0]?.id);
    expect(newState.lords[1]?.playerId).toEqual(newState.players[0]?.id);
    expect(newState.lords[2]?.playerId).toEqual(newState.players[1]?.id);
    expect(newState.lords[3]?.playerId).toEqual(newState.players[1]?.id);
  });

  test("should random lords order for 2 players", () => {
    // Arrange
    const id = `uuid-${Math.random()}`;
    ...
    // Assert
    expect(newState.lords[0]?.order).toEqual(1);
    expect(newState.lords[1]?.order).toEqual(2);
    expect(newState.lords[2]?.order).toEqual(3);
    expect(newState.lords[3]?.order).toEqual(4);
  });

  test("should random lords order for 3 players", () => {
    // Arrange
    const id = `uuid-${Math.random()}`;
